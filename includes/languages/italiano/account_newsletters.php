<?php
/*
  $Id$

  CE Phoenix, E-Commerce made Easy
  https://phoenixcart.org

  Copyright (c) 2022 Phoenix Cart - 2025 Sergio Strati

  Released under the GNU General Public License
*/

const NAVBAR_TITLE_1 = 'Il mio Account';
const NAVBAR_TITLE_2 = 'Sottoscrizioni alla Newsletter';

const HEADING_TITLE = 'Sottoscrizioni alla Newsletter';

const MY_NEWSLETTERS_GENERAL_NEWSLETTER = 'Newsletter Generale';
const MY_NEWSLETTERS_GENERAL_NEWSLETTER_DESCRIPTION = 'Include le notizie sul negozio, sui prodotti aggiunti, sulle offerte speciali e le priomizioni in corso.';

const SUCCESS_NEWSLETTER_UPDATED = 'Le tue sottoscritioni sono state aggiornate con successo.';
?>
